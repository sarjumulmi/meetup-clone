...
export const createMeetup = async (req, res) => {
  ...
  try {
    ...
    return res.status(e.status).json({ error: true, message: 'Error with Meetup create' });
  }
};
...
export const getAllMeetups = async (req, res) => {
  try {
    ...
    return res.status(e.status).json({ error: true, message: 'Error with Getting Meetup' });
  }
};
